...
Uses the macOS kernel sandbox (via ``sandbox-exec``) to enforce
filesystem, network, and process restrictions at the OS level.
Significantly stronger isolation than the plain local-subprocess backend,
with zero extra dependencies — works out of the box on any Mac.

.. note::

   ``sandbox-exec`` is marked deprecated by Apple, but the underlying
   kernel sandbox (Seatbelt / ``libsandbox``) is still used extensively
   by system services.  The CLI tool remains functional on macOS 15+.
"""
...
# ---------------------------------------------------------------------------
# Parameters injected at runtime via ``sandbox-exec -D``:
#   WORKSPACE  — the resolved real path of the temp workspace directory
#
# Policy: default-deny with minimal allowances for running Python/shell
...
;; ── debugging ──────────────────────────────────────────────────────────────
;; Uncomment the next line to log denied operations to the system log.
;; (debug deny)
...
;; ── filesystem reads (broad – needed for interpreter, libs, etc.) ─────────
(allow file-read*)

;; ── filesystem writes (restricted to workspace + temp) ─────────────────────
(allow file-write*
    (subpath (param "WORKSPACE"))
    (subpath "/private/tmp")
    (subpath "/private/var/folders"))    ;; macOS per-user temp dirs

;; ── mach IPC (minimal set for basic operation) ─────────────────────────────
(allow mach-lookup
    (global-name "com.apple.system.logger")
    (global-name "com.apple.system.notification_center"))

;; ── sysctl (read-only, needed by Python) ───────────────────────────────────
(allow sysctl-read)

;; ── network: DENIED by default (no allow network* rules) ──────────────────
"""

_SBPL_NETWORK_ADDENDUM = """\
;; ── network: ALLOWED ───────────────────────────────────────────────────────
(allow network*)
(allow mach-lookup
...
def _sandbox_exec_available() -> bool:
    """Return True if sandbox-exec is present and executable."""
    return _is_macos() and Path("/usr/bin/sandbox-exec").is_file()


class MacOSBackend(SandboxBackend):
    """Sandbox backend using macOS ``sandbox-exec`` (Seatbelt).

    Each :meth:`execute` call wraps the subprocess invocation with
    ``sandbox-exec -f <profile>``, enforcing kernel-level restrictions
    on filesystem writes, network access, and IPC.
    """
    ...
    def create(self, config: SandboxConfig) -> str:
        if not _sandbox_exec_available():
            ...
            raise SandboxException(
                "sandbox-exec is not available (requires macOS)"
            )
        ...
        # Write the SBPL profile into the workspace
        profile = _SBPL_PROFILE
        if config.network.enabled:
            profile += _SBPL_NETWORK_ADDENDUM
        self._profile_path = self._workspace / "sandbox.sb"
        self._profile_path.write_text(profile)

        self._created_at = self._now_iso()
        self._status = SandboxStatus.RUNNING
        self._log(f"macOS sandbox created: {self._id} at {self._workspace}")
        return self._id

    def destroy(self) -> None:
        if self._workspace and self._workspace.exists():
            shutil.rmtree(self._workspace, ignore_errors=True)
            self._log(f"macOS sandbox {self._id} destroyed")
    ...
    def execute(
        ...
        *,
        ...
        effective_timeout = timeout or (
            self._config.execution_timeout if self._config else 300
        )
    ...
    def get_info(self) -> SandboxInfo:
        return SandboxInfo(
            ...
            name=(
                self._config.name
                if self._config and self._config.name
                else f"macos-{self._id or 'none'}"
            ),
            status=self._status,
